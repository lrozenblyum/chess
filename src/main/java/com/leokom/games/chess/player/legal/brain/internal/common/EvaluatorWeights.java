...
public final class EvaluatorWeights {
	...
	private static Map<EvaluatorType, Double > getStandardWeights() {
		...
		result.put( EvaluatorType.TERMINAL, HIGHEST_PRIORITY );
		result.put( EvaluatorType.CASTLING_SAFETY, NORMAL_PRIORITY );
		result.put( EvaluatorType.CENTER_CONTROL, NORMAL_PRIORITY );
		result.put( EvaluatorType.MOBILITY, NORMAL_PRIORITY );
		result.put( EvaluatorType.MATERIAL, INCREASED_PRIORITY );
		result.put( EvaluatorType.PROTECTION, NORMAL_PRIORITY );
		result.put( EvaluatorType.ATTACK, NORMAL_PRIORITY );
