...
import static com.leokom.games.chess.utils.CollectionUtils.filterMapByValues;
...
public class Position implements GameState< Move, Position > {
	...
	private Set<String> generatePossibleCastlingDestinations( String square ) {
		...
		if ( square.equals( Board.square( kingFile, castlingRank ) ) ) {
			...
			if ( isOccupiedBy( Board.square( kingSideRookFile, castlingRank ), side ) &&
				...
				!isSquareAttacked( side, Board.square( 'f', castlingRank ) ) &&
				isFreeRankBetween( kingFile, kingSideRookFile, castlingRank )) {
				result.add( Board.square( 'g', castlingRank ) );
			}
			...
			if ( isOccupiedBy( Board.square( queenSideRookFile, castlingRank ), side ) &&
				...
				!isSquareAttacked( side, Board.square( 'd', castlingRank ) ) &&
				isFreeRankBetween( queenSideRookFile, kingFile, castlingRank )) {
				result.add( Board.square( 'c', castlingRank ) );
			}
		}
	...
	private boolean isFreeRankBetween( char leftFile, char rightFile, int rank ) {
		return Board.getSquaresBetween( leftFile, rightFile, rank )
	...
	private Set<String> getSquaresAttackedByKing( String square ) {
		...
		for ( HorizontalDirection horizontalDirection : HorizontalDirection.all() ) {
			for ( VerticalDirection verticalDirection : VerticalDirection.all() ) {
				Board.squareDiagonally( square, horizontalDirection, verticalDirection ).
		...
		for ( Direction direction : Direction.all() ) {
			Board.squareTo( square, direction )
			.ifPresent( result::add );
		}
	...
	private Set<String> getSquaresAttackedByRook( String square ) {
		...
		for ( Direction direction : Direction.all() ) {
			...
			do {
				runningSquare = Board.squareTo( runningSquare.get(), direction );
	...
	private Set<String> getSquaresAttackedByBishop( String square ) {
		...
		for ( HorizontalDirection horizontalDirection : HorizontalDirection.all() ) {
			for ( VerticalDirection verticalDirection : VerticalDirection.all() ) {
				...
				do {
					movingSquare = Board.squareDiagonally( movingSquare.get(), horizontalDirection, verticalDirection );
	...
	private Stream< String > getSquaresAttackedByPawn( String square ) {
		return HorizontalDirection.all().stream()
			.map( horizontalDirection -> {
				...
				return Board.squareDiagonally( square, horizontalDirection, getPawnMovementDirection( side ) );
	...
	private Set<String> getPawnMoves( String square ) {
		...
		final char file = Board.fileOfSquare( square );
		...
		if ( rank == getRankBeforePromotion( side ) ) {
			...
			getSquaresAttackedByPawn( square )
				...
				.flatMap( attackedSquare ->
					getPromotionResult( Board.fileOfSquare( attackedSquare ), side )
				)
				.forEach( result::add );
		}
		else {
	...
	private boolean canPawnMove( String square, String potentialMove ) {
		...
		return !Board.sameFile( destinationSquare, square ) ||
	...
	private boolean isPawnDoubleMoveProhibited( String square, String destinationSquare ) {
		...
		return Board.rankOfSquare( destinationSquare ) == getDoubleMoveRank( side ) &&
				Board.rankOfSquare( square ) == getPawnInitialRank( side )
				&& isOccupied( Board.square( Board.fileOfSquare( square ), intermediateRank ) );
	}
