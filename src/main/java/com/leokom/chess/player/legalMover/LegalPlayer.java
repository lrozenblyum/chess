...
public class LegalPlayer implements Player {
	...
	private double evaluateMove( Move move ) {
		...
		List< Evaluator > evaluators = Arrays.asList( new CastlingSafetyEvaluator(), new CenterControlEvaluator() );
		double estimate = 0;
		for ( Evaluator evaluator : evaluators ) {
			// rather safe if all evaluators keep convention [0 , 1]
			//here we also can multiply by wight of each evaluator for us
